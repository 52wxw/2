        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div class="relative">
            ...
            >
              ...
              <option value="private_cloud">{{ $t('pages.taskList.categories.privateCloud') }}</option>
              <option value="public_cloud">{{ $t('pages.taskList.categories.publicCloud') }}</option>
              <option value="container_cloud">{{ $t('pages.taskList.categories.containerCloud') }}</option>
            </select>
    ...
    <div class="container mx-auto px-4 mt-8">
      ...
      <div v-else-if="filteredTasks.length === 0" class="text-center py-12">
        <div class="inline-flex items-center justify-center w-20 h-20 bg-gray-800 rounded-full mb-4">
          <i class="fa-solid fa-search-plus text-primary text-3xl"></i>
        </div>
...
const filteredTasks = computed(() => {
  ...
  // 按解决状态排序（已解决的排在前面）
  tasks.sort((a, b) => {
    const solvedTasks = userStore.solvedTasks || [];
    const aSolved = solvedTasks.includes(a.id);
    const bSolved = solvedTasks.includes(b.id);
