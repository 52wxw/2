<template>
  <div class="bg-gray-900 min-h-screen text-gray-100">
    ...
    <!-- 空状态 -->
    <div v-if="!loading && !errorMessage && users.length === 0" class="text-center py-16">
...
const selectedUserIds = ref(new Set()); // 存储选中的用户 ID
...
const getUserRoleText = (user) => {
  return getUserRole(user) === 'ROLE_ADMIN' ? '管理员' : '普通用户';
};
...
const fetchUsers = async () => {
  ...
  try {
    ...
    totalUsers.value = response.length || 0; // 总数量为数组长度
